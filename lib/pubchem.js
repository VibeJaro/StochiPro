...
const DEFAULT_TIMEOUT_MS = Number.parseInt(process.env.PUBCHEM_TIMEOUT_MS || '', 10) || 8000;
...
function createTimeoutError(timeoutMs) {
  const error = new Error(`PubChem request timed out after ${timeoutMs}ms`);
  error.code = 'PUBCHEM_TIMEOUT';
  return error;
}
...
async function fetchJson(url, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      ...
      const timeout = reason instanceof Error ? reason : timeoutError;
      timeout.code = timeout.code || 'PUBCHEM_TIMEOUT';
      throw timeout;
    }
    throw error;
  } finally {
...
async function fetchJsonWithTrace(url, entry, stage, { rethrow = true, timeoutMs } = {}) {
...
async function fetchCidForName(query, timeoutMs) {
...
async function fetchSynonym(cid, timeoutMs) {
...
async function fetchProperties(cid, timeoutMs) {
...
function extractCasNumber(viewData, synonyms = []) {
  const casRegex = /\b\d{2,7}-\d{2}-\d\b/;
  const matches = [];

  const collectStrings = (section) => {
    if (!section) return;
    extractStrings(section).forEach((val) => matches.push(val));
    (section.Section || []).forEach(collectStrings);
  };

  const sections = viewData?.Record?.Section || [];
  sections.forEach(collectStrings);
  synonyms.forEach((val) => matches.push(val));

  for (const entry of matches) {
    const match = String(entry).match(casRegex);
...
async function fetchPhysicalProperties(cid, timeoutMs) {
  const viewUrl = `${BASE_URL}_view/data/compound/${cid}/JSON`;
  const viewData = await fetchJson(viewUrl, { timeoutMs }).catch(() => null);
...
async function searchCompound(query, trace, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    const cas = extractCasNumber(viewData, synonyms);
