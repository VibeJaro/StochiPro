...
async function enrichWithChemData(
  ...
  const retryName = await retryFn(component.name, input, retryPrompt, logs, {
    extractionContext,
    attemptedNames: names,
    knownComponents
  });
...
async function analyzeText(input, prompts = {}, adapters = {}) {
  ...
  const llmExtraction = await extractFn(input, promptSet.primaryPrompt, log);
  const extractionContext = llmExtraction.raw || '';
  ...
  if (!components.length) {
    ...
    return { logs, components: [], summary: 'Keine Komponenten gefunden.' };
  }
  const enriched = [];
  const knownComponents = components.map((c) => c.name).filter(Boolean);
  for (const component of components) {
    ...
    const enrichedComponent = await enrichWithChemData(
      ...
      null,
      extractionContext,
      knownComponents.filter((name) => name && name !== component.name)
    );
