...
function calculateStoichiometry(components = []) {
  const normalized = components.map((component) => ({
    ...component,
    quantity: component.amount?.value ?? component.quantity ?? null,
    unit: component.amount?.unit ?? component.unit ?? null,
    molecularWeight: component.pubchem?.molecularWeight ?? component.molecularWeight,
    density: component.pubchem?.density ?? component.density,
    coefficient: component.coefficient || 1,
    role: component.role || 'edukt'
  }));

  enrichStoichiometry(normalized);

  return normalized.map((component) => ({
    ...component,
    calculated: {
      mass: component.mass,
      mmol: component.moles,
...
export { enrichStoichiometry, toNumber, calculateStoichiometry };
